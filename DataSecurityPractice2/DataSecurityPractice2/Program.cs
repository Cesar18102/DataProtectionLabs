...
namespace DataSecurityPractice2
{
    public class DisposableNotepad
    {
        ...
        public string Decode(string encoded, int key) =>
            DecodeCode(encoded, key.ToString());
    ...
    public class Program
    {
        ...
        {
            DisposableNotepad notepad = new DisposableNotepad();

            while (true)
            {
                Console.WriteLine("Input a text to encode: ");
                string text = Console.ReadLine();

                Console.WriteLine("Input a key for encoding: ");
                string key = Console.ReadLine();

                int ikey = 0;
                bool keyNum = int.TryParse(key, out ikey);

                int itext = 0;
                bool textNum = int.TryParse(text, out itext);

                string encoded = "";

                if (textNum && keyNum)
                    encoded = notepad.Encode(itext, ikey);
                else if (!textNum && !keyNum)
                    encoded = notepad.Encode(text, key);
                else if (textNum)
                    encoded = notepad.Encode(itext, key);
                else if (keyNum)
                    encoded = notepad.Encode(text, ikey);
                else
                    encoded = "ERROR";

                Console.WriteLine("Encoded: " + encoded);
                Console.WriteLine("Encoded text: " + notepad.GetCodeText(encoded));

                string decoded = (keyNum ? notepad.Decode(encoded, ikey) : notepad.Decode(encoded, key));
                Console.WriteLine("Decoded: " + decoded);
                Console.WriteLine("Decoded text: " + notepad.GetCodeText(decoded));
